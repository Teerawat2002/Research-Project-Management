<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-50 dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white">หัวข้อรอง</h3>

                    <div class="mt-4">
                        <!-- Filter Form -->
                        <div
                            class="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0 sm:space-x-4 pb-4 mt-3">
                            {{-- Filter + Search --}}
                            <form id="filterForm" method="GET" action="{{ route('admin.topic.subtopic.index') }}"
                                ...
                                {{-- Dropdown เลือกหัวข้อหลัก --}}
                                <select name="maintopic_id" id="maintopic_id"
                                    class="block w-full sm:w-48 px-4 py-2 text-sm border rounded-md bg-white dark:bg-gray-700 dark:text-white">
                                    <option value="">หัวข้อทั้งหมด</option>
                                    <option value="unattached"
                                        {{ request('maintopic_id') === 'unattached' ? 'selected' : '' }}>
                                        ไม่อยู่ในหัวข้อหลัก
                                    </option>
                                    @foreach ($maintopics as $mt)
                                        <option value="{{ $mt->id }}"
                                            {{ request('maintopic_id') == $mt->id ? 'selected' : '' }}>
                                            {{ $mt->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <div class="relative w-full sm:w-auto">
                                    ...
                                    <input type="text" name="search" id="table-search"
                                        ...
                                        class="block w-full sm:w-64 p-2 pl-10 text-sm border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                            ...
                            <div class="w-full sm:w-auto">
                                <a href="{{ route('admin.topic.subtopic.create') }}"
                                    class="block w-full sm:inline-block text-center bg-blue-500 hover:bg-blue-700 text-white font-medium rounded-lg text-sm px-5 py-2.5 transition">
                                    ...
                                    @forelse ($sub_topics as $topic)
                                        <tr
                                            ...
                                            <td class="px-6 py-4 text-center truncate">
                                                {{-- <a href="{{ route('admin.topic.subtopic.edit', $topic->id) }}"
                                            ...
                                            <form action="{{ route('admin.topic.subtopic.delete', $topic->id) }}"
                                                ...
                                                <button type="button" title="แก้ไข"
                                                    onclick="window.location.href='{{ route('admin.topic.subtopic.edit', $topic->id) }}'"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const maintopicSelect = document.getElementById('maintopic_id');
            ...
            // Debounce search input to submit form after user stops typing
            // let debounceTimer;
