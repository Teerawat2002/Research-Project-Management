    ...
    <div class="mt-16 py-8">
        <div class="p-6 text-gray-900">
            <div class="max-w-md mx-auto bg-gray-50 dark:bg-gray-800 p-6 shadow-lg rounded-lg">
                <h1 class="text-2xl text-gray-900 dark:text-white font-bold mb-4">แก้ไขหัวข้อรอง</h1>
                ...
                <form action="{{ route('admin.topic.subtopic.update', $subTopic->id) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <!-- เลือกประเภทหัวข้อ -->
                    <label for="mtopic_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">เลือกหัวข้อหลัก</label>
                    <select name="mtopic_id" id="mtopic_id"
                        ...
                        @foreach ($main_topics as $main_topic)
                            <option value="{{ $main_topic->id }}" {{ old('mtopic_id', $subTopic->mtopic_id) == $main_topic->id ? 'selected' : '' }}>
                                {{ $main_topic->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <input type="text" id="name" name="name"
                        ...
                        placeholder="ใส่ชื่อหัวข้อ" required value="{{ old('name', $subTopic->name) }}">
                    ...
                    <input type="text" id="score" name="score"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="ใส่คะแนน" required value="{{ old('score', $subTopic->score) }}">
