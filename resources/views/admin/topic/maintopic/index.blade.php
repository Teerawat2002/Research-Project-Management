<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-50 dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white">หัวข้อหลัก</h3>
           ...
           w-full sm:w-auto space-y-2 sm:space-y-0 sm:space-x-2">
             ...
             bg-white dark:bg-gray-700 dark:text-white">
                                    ...
                                    <option value="unattached"
                                        {{ request('form_id') === 'unattached' ? 'selected' : '' }}>
                                        ไม่อยู่ในแบบฟอร์ม
                                    </option>
                                    @foreach ($forms as $f)
                                        <option value="{{ $f->id }}"
                                            {{ request('form_id') == $f->id ? 'selected' : '' }}>
                                            {{ $f->name }}
                                        </option>
                                    @endforeach
                                </select>
               ...
               dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                            ...
                            <div class="w-full sm:w-auto">
                                <a href="{{ route('admin.topic.maintopic.create') }}"
             ...
             font-medium rounded-lg text-sm px-5 py-2.5 transition">
                        ...
                        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    @forelse ($main_topics as $topic)
                                        <tr
                                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            <th scope="row"
                                                class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                {{ $loop->iteration + ($main_topics->currentPage() - 1) * $main_topics->perPage() }}
                                            </th>
                                            <td class="px-6 py-4">
                                                {{ $topic->id }}
                                            </td>
                                        ...
                                        <td class="px-6 py-4">
                                            ...
                                            {{-- <td class="px-6 py-4">
                                            <a href="{{ route('admin.topic.maintopic.edit', $topic->id) }}"
                                                class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                            <form action="{{ route('admin.topic.maintopic.delete', $topic->id) }}"
                                            ...
                                            <form action="{{ route('admin.topic.maintopic.delete', $topic->id) }}"
                                                ...
                                                <button type="button" title="แก้ไข"
                                                    onclick="window.location.href='{{ route('admin.topic.maintopic.edit', $topic->id) }}'"
    ...
    <!-- Script for auto-submission of the filter form -->
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Submit form when the user changes the dropdown
            formSelect.addEventListener('change', () => {
                filterForm.submit();
            });

            // Debounce the search input so it doesn't submit on every keystroke
            // let timer;
